...
export const WindDataColumn = ({ entry, showText, onToggleDirection, isLast }: WindDataColumnProps) => {
    ...
    return (
        <div className={`min-w-12 max-w-12 space-y-0.5 ${!isLast ? 'mr-0.5' : ''}`}>
            <span className="flex items-center justify-center p-1.5 font-bold bg-zinc-200 dark:bg-zinc-800">
                {entry.time}
            </span>
            ...
            <span
                className="flex flex-col items-center justify-center p-1.5 font-bold bg-zinc-200 dark:bg-zinc-800 cursor-pointer select-none transition-all duration-150"
                title={`${windDir}`}
